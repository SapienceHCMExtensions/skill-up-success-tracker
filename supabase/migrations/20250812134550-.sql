-- Multi-tenancy migration (fixed order)
-- 0) Create organizations table first
...
DECLARE c int; BEGIN
  ...
  IF c = 0 THEN
    INSERT INTO public.organizations(name, subdomain) VALUES ('Default Organization','default');
  END IF;
END$$;

-- 1) Add organization_id to employees first, backfill, and constrain
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='employees' AND column_name='organization_id'
  ) THEN
...
DO $$
DECLARE dorg uuid; BEGIN
  SELECT id INTO dorg FROM public.organizations ORDER BY created_at ASC LIMIT 1;
...
-- 2) Now create helper to get current user's org
...
-- 3) Add organization_id to user_roles next, backfill, and constrain
DO $$ BEGIN
  IF NOT EXISTS (
...
-- 4) Create org-scoped role check now that user_roles has org
...
-- 5) Add organization_id to remaining tables and backfill
DO $$ BEGIN
  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='departments' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.departments ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='organization_settings' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.organization_settings ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='email_templates' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.email_templates ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='courses' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.courses ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='plans' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.plans ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='plan_modules' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.plan_modules ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='plan_resources' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.plan_resources ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='plan_trainers' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.plan_trainers ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='plan_employees' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.plan_employees ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='plan_evaluations' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.plan_evaluations ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='sessions' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.sessions ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='session_enrollments' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.session_enrollments ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='training_requests' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.training_requests ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='course_cost_actuals' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.course_cost_actuals ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='cost_actuals' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.cost_actuals ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='evaluation_responses' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.evaluation_responses ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='course_evaluation_responses' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.course_evaluation_responses ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='external_trainers' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.external_trainers ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='training_vendors' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.training_vendors ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='training_audit' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.training_audit ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='security_audit_logs' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.security_audit_logs ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='workflow_definitions' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.workflow_definitions ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='workflow_instances' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.workflow_instances ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='workflow_tasks' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.workflow_tasks ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='workflow_instance_events' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.workflow_instance_events ADD COLUMN organization_id uuid; END IF;

  PERFORM 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='employee_certificates' AND column_name='organization_id';
  IF NOT FOUND THEN ALTER TABLE public.employee_certificates ADD COLUMN organization_id uuid; END IF;
END$$;
...
-- Constrain and index remaining tables
...
-- 6) Org-scoped RLS policies (drop old, add new)
...
-- 7) Update handle_new_user to auto-create/assign orgs and grant admin to first member
